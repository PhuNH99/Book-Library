...
import { DeleteOutlined, EditOutlined, ExclamationCircleOutlined, RightOutlined, SoundOutlined } from '@ant-design/icons';
...
import { fetchAllBookInBookGroup } from '@/services/bookgroup';
...
class ViewForm extends React.Component<ViewFormProps, ViewFormState> {
          ...
          <Col span={12} style={{ paddingTop: 5, textAlign: 'right' }}>
            {this.props.user.currentUser.role != 1 ? (
              <Space className={styles.stickButton}>
                ...
                <Popconfirm title="Are you sure?" placement="bottomLeft" onConfirm={this.handelDelete}>
                  <Button
                    className={styles.buttonCustom}
  ...
  async handelDelete() {
    ...
    const response = await fetchAllBookInBookGroup(bookGroup.id);
    if (response.data.length != 0) {
      Modal.warning({
        centered: true,
        title: 'There are book that been borrow !',
        icon: <ExclamationCircleOutlined />,
        content: 'Make sure every book is available or been remove to delete bookgroup',
      ...
      dispatch({
        ...
        sendNotification('Delete Success !', `Successfull delete bookgroup`, 'success');
        dispatch({
          type: 'bookgrouptable/fetchData',
          payload: {
